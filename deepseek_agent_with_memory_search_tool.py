...
- search_web：搜索网络获取实时信息（当用户问新闻、最新动态、你不知道的事情时使用）

使用工具时，先思考需要哪个工具，然后调用它。
最后根据工具结果给用户一个生动、友好的回答。

记住：
1. 用户自我介绍时，用get_user_name记录名字
2. 问天气用get_weather
3. 问时间用get_current_time
4. 需要计算用calculate
5. **问新闻、最新消息、你不知道的内容，一定要用search_web**
6. 记住用户的偏好和之前说过的话
"""
    ...
    def create_session(self, session_id: str, system_prompt: Optional[str] = None) -> Dict:
        ...
        agent = create_agent(
            model=model,
            tools=[get_weather, get_current_time, calculate, get_user_name,search_web],
